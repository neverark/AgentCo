...
function RunReport({ run }: { run: Run }) {
  ...
  return <>
    <div className={`report-banner ${run.finalStatus}`}><FileCheck2 size={30} /><div><h3>{run.finalStatus === 'completed' ? 'A clearer picture. A complete paper trail.' : 'Every outcome stays accountable.'}</h3><p>{run.finalStatus === 'completed' ? 'Your sample token risk snapshot is ready.' : run.frames.at(-1)?.detail}</p></div></div>
    ...
    <div className="note"><Info size={16} /><p>Checks cover the displayed facts only. They do not establish token safety or constitute an audit. The provider findings here are sample records, separate from live token metadata.</p></div>
...
export default function App() {
  ...
  return <div className="app-shell">
      ...
      <main id="main-content">
        ...
        {view === 'workspace' && <div className="workspace-grid">
          ...
          <aside className="workspace-aside"><section className="treasury-card"><div className="treasury-arc" aria-hidden="true" /><div className="treasury-title"><span><Wallet size={17} /> Treasury guard</span><ShieldCheck size={17} /></div><div className="budget-display"><span>YOUR SPENDING LIMIT</span><strong><Money cents={safeBudget} /><small>USDT</small></strong><p>{finished ? 'Every cent, accounted for.' : 'A hard limit. A smarter allocation.'}</p></div>
              ...
              <p className="no-funds"><FlaskConical size={12} /> Sample catalog · modeled charges</p>
            </section>
    ...
    {overlay?.kind === 'report' && <Modal title={`${overlay.run.task.tokenSymbol} · Risk snapshot`} eyebrow={`${overlay.run.id} / SAMPLE REPORT`} onClose={() => setOverlay(null)} wide><RunReport run={overlay.run} /></Modal>}
